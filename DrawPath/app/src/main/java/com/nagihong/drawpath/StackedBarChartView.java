...
public class StackedBarChartView extends View {
    ...
    private String Tag = StackedBarChartView.class.getSimpleName();
     ...
     */
    private StackedBarChartDataSet mDataSet = new StackedBarChartDataSet();
    ...
    private void performClick(MotionEvent event) {
        ...
        for (int j = 0; j < mDataSet.getDataSetCount(); j++) {
            Data data = mDataSet.getDataSet().get(j).get(mSelectedDataIndex);
            if (realPoint[1] > data.getY()) {
