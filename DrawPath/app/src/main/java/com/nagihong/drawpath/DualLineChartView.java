...
import com.nagihong.drawpath.DataSet.DualLineChartDataSet;
...
public class DualLineChartView extends View {

    private String Tag = DualLineChartView.class.getSimpleName();
    ...
    private int mUpperColor = ColorList.getColors().get(2);
    private int mLowerColor = ColorList.getColors().get(5);
     ...
     */
    private Paint mLinePaint, mBitmapPaint, mPointPaint, mValuePaint;
     ...
     */
    private DualLineChartDataSet mDataSet = new DualLineChartDataSet();
    ...
    public DualLineChartView(Context context) {
        super(context);
        initVar();
    }

    public DualLineChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initVar();
    }

    public DualLineChartView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initVar();
    }

    public DualLineChartView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas canvas) {
        Log.d(Tag, "onDraw() -- count : " + countDraw++);
        super.onDraw(canvas);
    ...
    private void drawFrame(Canvas canvas) {
        ...
        mFramePaint.setColor(mLowerColor);
        canvas.drawLine(mPaddingLeft, mPaddingTop, mPaddingLeft, canvas.getHeight() - mPaddingBottom, mFramePaint);
        mFramePaint.setColor(mUpperColor);
        canvas.drawLine(canvas.getWidth() - mPaddingRight, mPaddingTop, canvas.getWidth() - mPaddingRight, canvas.getHeight() - mPaddingBottom, mFramePaint);
    }
    ...
    private void drawGridLines(Canvas canvas) {
        ...
        drawLowerYAxisGridLines(canvas);
        drawUpperYAxisGridLines(canvas);
    }
    ...
    private void drawUpperYAxisGridLines(Canvas canvas) {
        mGridPaint.setColor(mUpperColor);
        float[] ys = mDataSet.calcUpperYAxisGridLinesY(getFrameHeight(), getYAxisScale());
        for (int i = 0; i < ys.length; i++) {
            ...
            canvas.drawLine(mPaddingLeft + getFrameWidth() / 2, getMatrixPoint()[1] + mPaddingTop, canvas.getWidth() - mPaddingRight, getMatrixPoint()[1] + mPaddingTop, mGridPaint);
        }
        drawUpperYAxisMark(canvas, ys);
    }

    private void drawLowerYAxisGridLines(Canvas canvas) {
        mGridPaint.setColor(mLowerColor);
        float[] ys = mDataSet.calcLowerYAxisGridLinesY(getFrameHeight(), getYAxisScale());
        for (int i = 0; i < ys.length; i++) {
            getMatrixPoint()[1] = ys[i];
            getGlobalMatrix().mapPoints(getMatrixPoint());
            if(getMatrixPoint()[1] < 0) {
            ...
            canvas.drawLine(mPaddingLeft, getMatrixPoint()[1] + mPaddingTop, getFrameWidth() / 2 + mPaddingLeft, getMatrixPoint()[1] + mPaddingTop, mGridPaint);
        }
        drawLowerYAxisMark(canvas, ys);
    }
    ...
    private void drawLowerYAxisMark(Canvas canvas, float[] ys) {
        mMarkPaint.setColor(mLowerColor);
        float characterHeight = calcCharacterHeight(mMarkPaint);
        float characterWidth = calcCharacterWidth(mMarkPaint);
        float yAxisIncrementVal = mDataSet.calcLowerYAxisMarkIncrementVal(getYAxisScale());
        float maxVal = (int) mDataSet.calcLowerMaxVal(getFrameHeight());
        boolean showDecimal = false;
        if(mDataSet.calcLowerYAxisMarkIncrementVal(getYAxisScale()) < 1f) {
            showDecimal = true;
        }
        for (int i = -1; i < ys.length; i++) {
            if (i == -1) {
                ...
                canvas.drawText(markS, mPaddingLeft - characterXOffset, mPaddingTop + getMatrixPoint()[1] + characterHeight / 2, mMarkPaint);
            }
        }
    }

    private void drawUpperYAxisMark(Canvas canvas, float[] ys) {
        mMarkPaint.setColor(mUpperColor);
        float characterHeight = calcCharacterHeight(mMarkPaint);
        float characterWidth = calcCharacterWidth(mMarkPaint);
        float yAxisIncrementVal = mDataSet.calcUpperYAxisMarkIncrementVal(getYAxisScale());
        float maxVal = (int) mDataSet.calcUpperMaxVal(getFrameHeight());
        boolean showDecimal = false;
        if(mDataSet.calcUpperYAxisMarkIncrementVal(getYAxisScale()) < 1f) {
            showDecimal = true;
        }
        for (int i = -1; i < ys.length; i++) {
            if (i == -1) {
                ...
                float characterXOffset = FloatFormatter.format(maxVal).length() * characterWidth;
                canvas.drawText(FloatFormatter.format(maxVal), mPaddingLeft + getFrameWidth() + 10, mPaddingTop + getMatrixPoint()[1] + characterHeight / 2, mMarkPaint);
            } else {
                getMatrixPoint()[1] = ys[i];
                getGlobalMatrix().mapPoints(getMatrixPoint());
                if(getMatrixPoint()[1] < 0) {
                ...
                String markS = FloatFormatter.format(mark);
                float characterXOffset = markS.length() * characterWidth;
                canvas.drawText(markS, mPaddingLeft + getFrameWidth() + 10, mPaddingTop + getMatrixPoint()[1] + characterHeight / 2, mMarkPaint);
            }
        }
    }
    ...
    private void drawData(Canvas canvas) {
        ...
        drawUpperDataLines(mBitmapCanvas);
        drawLowerDataLines(mBitmapCanvas);
        drawUpperDataPoints(mBitmapCanvas);
        drawLowerDataPoints(mBitmapCanvas);
        ...
        if (isNeedToDrawDataValues()) {
            drawUpperDataValues(canvas);
            drawLowerDataValues(canvas);
        }
    }

    private void drawUpperDataLines(Canvas bitmapCanvas) {
        mLinePaint.setColor(mUpperColor);
        mLinePaint.setStrokeWidth(3);
        mLinePaint.setAntiAlias(true);
        int pointCount = (mDataSet.getUpperDatasCount() - 1) * 2;
        int pointArrLength = pointCount * 2;
        float[] mLineBuffer = new float[pointArrLength];
        int index = 0;
        for (int i = 0; i < mDataSet.getUpperDatasCount(); i++) {
            Data data = mDataSet.getUpperDatas().get(i);
            if (i == 0 || i == mDataSet.getUpperDatasCount() - 1) {
    ...
    private void drawLowerDataLines(Canvas bitmapCanvas) {
        mLinePaint.setColor(mLowerColor);
        mLinePaint.setStrokeWidth(3);
        mLinePaint.setAntiAlias(true);
        int pointCount = (mDataSet.getLowerDatasCount() - 1) * 2;
        int pointArrLength = pointCount * 2;
        float[] mLineBuffer = new float[pointArrLength];
        int index = 0;
        for (int i = 0; i < mDataSet.getLowerDatasCount(); i++) {
            Data data = mDataSet.getLowerDatas().get(i);
            if (i == 0 || i == mDataSet.getLowerDatasCount() - 1) {
                mLineBuffer[index++] = data.getX();
                mLineBuffer[index++] = data.getY();
            } else {
                mLineBuffer[index++] = data.getX();
    ...
    private void drawUpperDataPoints(Canvas bitmapCanvas) {
        mPointPaint.setStyle(Paint.Style.FILL);
        mPointPaint.setColor(mUpperColor);
        for (int i = 0; i < mDataSet.getUpperDatasCount(); i++) {
            Data data = mDataSet.getUpperDatas().get(i);
            ...
            bitmapCanvas.drawCircle(getMatrixPoint()[0], getMatrixPoint()[1], 8, mPointPaint);
        }
    }

    private void drawLowerDataPoints(Canvas bitmapCanvas) {
        mPointPaint.setStyle(Paint.Style.FILL);
        mPointPaint.setColor(mLowerColor);
        for (int i = 0; i < mDataSet.getLowerDatasCount(); i++) {
            Data data = mDataSet.getLowerDatas().get(i);
            getMatrixPoint()[0] = data.getX();
            getMatrixPoint()[1] = data.getY();
    ...
    private void drawUpperDataValues(Canvas canvas) {
        ...
        for (int i = 0; i < mDataSet.getUpperDatasCount(); i++) {
            ...
            float startX = getMatrixPoint()[0] - characterXOffset + mPaddingLeft;
            float startY = getMatrixPoint()[1] - 15 + mPaddingTop;
            canvas.drawText(data.getFormattedValue(), startX, startY, mValuePaint);
        }
    }

    private void drawLowerDataValues(Canvas canvas) {
        int characterWidth = calcCharacterWidth(mValuePaint);
        int characterHeight = calcCharacterHeight(mValuePaint);
        for (int i = 0; i < mDataSet.getLowerDatasCount(); i++) {
            Data data = mDataSet.getLowerDatas().get(i);
    ...
    private boolean isNeedToDrawDataValues() {
        ...
        getGlobalMatrix().mapPoints(getMatrixPoint());
        x0 = getMatrixPoint()[0];
        getMatrixPoint()[0] = mDataSet.calcXAxisIncrementLength(getFrameWidth());
        getGlobalMatrix().mapPoints(getMatrixPoint());
        x1 = getMatrixPoint()[0];
        if (Math.abs(x1 - x0) >= calcCharacterWidth(mValuePaint) * 1.5f) {
            return true;
        } else {
            return false;
        }
