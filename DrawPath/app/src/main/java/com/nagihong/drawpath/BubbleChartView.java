...
import com.nagihong.drawpath.DataSet.BubbleChartDataSet;
...
public class BubbleChartView extends View {
    ...
    private String Tag = BubbleChartView.class.getSimpleName();
     ...
     */
    private Paint mCirclePaint, mBitmapPaint;
     ...
     */
    private BubbleChartDataSet mDataSet = new BubbleChartDataSet();
    ...
    public BubbleChartView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void drawData(Canvas canvas) {
        ...
        drawCircles(mBitmapCanvas);
        canvas.drawBitmap(mDrawBitmap.get(), mPaddingLeft, mPaddingTop, mBitmapPaint);
    }

    private void drawCircles(Canvas bitmapCanvas) {
        for (int i = 0; i < mDataSet.getDataSetCount(); i++) {
            List<Data> dataSet = mDataSet.getDatas().get(i);
            for (int j = 0; j < mDataSet.getCount(); j++) {
                Data data = dataSet.get(j);
                getMatrixPoint()[0] = data.getX();
                getMatrixPoint()[1] = data.getY();
                getGlobalMatrix().mapPoints(getMatrixPoint());
                float radius = data.getR() * getYAxisScale();
                if (getMatrixPoint()[0] + radius < 0
                        || getMatrixPoint()[1] + radius < 0
                        || getMatrixPoint()[1] - radius > getFrameHeight()) {
                    continue;
                }
                if (getMatrixPoint()[0] - radius > getFrameWidth()) {
                    break;
                }
                mCirclePaint.setStyle(Paint.Style.FILL);
                mCirclePaint.setColor(data.getColor());
                mCirclePaint.setAlpha(200);
                bitmapCanvas.drawCircle(getMatrixPoint()[0], getMatrixPoint()[1], radius, mCirclePaint);
                if(i == mSelectedDataSetIndex
                        && j == mSelectedDataIndex) {
                    mCirclePaint.setStyle(Paint.Style.STROKE);
                    mCirclePaint.setStrokeWidth(10);
                    mCirclePaint.setColor(data.getColor());
                    mCirclePaint.setAlpha(255);
                    bitmapCanvas.drawCircle(getMatrixPoint()[0], getMatrixPoint()[1], radius + 10, mCirclePaint);
                }
            }
        }
    }
    ...
    private void performClick(MotionEvent event) {
        ...
        for (int j = 0; j < mDataSet.getDataSetCount(); j++) {
            List<Data> dataSet = mDataSet.getDatas().get(j);
            int startI = (mSelectedDataIndex > 1) ? mSelectedDataIndex - 1 : mSelectedDataIndex;
            int endI = (mSelectedDataIndex == mDataSet.getCount() - 1) ? mDataSet.getCount() - 1 : mSelectedDataIndex + 1;
            for (int i = startI; i <= endI; i++) {
                Data data = dataSet.get(i);
                if(spacing(realPoint[0], realPoint[1], data.getX(), data.getY()) < data.getR()) {
                    mSelectedDataIndex = i;
                    mSelectedDataSetIndex = j;
                    invalidate();
                    return;
                }
            }
